    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
            ...
            <span>模型 {data.modelUsed}</span>
            <span>抽帧每 {data.frameIntervalSec}s · {data.frames.length} 帧</span>
      ...
      {(data.status === "analyzing" || data.status === "uploading") && (
        <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900">
          视觉模型正在对照检查项审阅抽帧，完成后会写入本报告，可随时回看。
        </div>
      )}
      ...
      <section>
        <h2 className="mb-3 font-medium">抽帧时间轴</h2>
        <div className="flex gap-3 overflow-x-auto pb-2">
          {data.frames.map((frame) => (
            <figure key={frame.id} className="w-36 shrink-0">
              <img src={frame.url} alt={`t=${frame.timestampSec}`} className="h-24 w-36 rounded-lg object-cover ring-1 ring-slate-200" />
              <figcaption className="mt-1 text-center text-xs text-slate-500">{formatTime(frame.timestampSec)}</figcaption>
            </figure>
          ))}
        </div>
      </section>
        ...
        {data.items.map((item) => (
          <article key={item.id} className="rounded-2xl bg-white p-5 ring-1 ring-slate-200">
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                <div className="text-xs text-teal">步骤 {item.stepOrder} {item.category ? `· ${item.category}` : ""}</div>
            ...
            <div className="mt-3 flex flex-wrap gap-4 text-xs text-slate-400">
              {item.result?.observedAtSec != null && <span>观察到 {formatTime(item.result.observedAtSec)}</span>}
              {item.result?.confidence != null && <span>置信度 {Math.round(item.result.confidence * 100)}%</span>}
              {item.passCriteria && <span>标准：{item.passCriteria}</span>}
            </div>
            {item.evidence.length > 0 && (
              <div className="mt-3 flex gap-2">
                {item.evidence.map((frame) => (
                  <img key={frame.id} src={frame.url} alt="证据帧" className="h-20 w-28 rounded-md object-cover ring-1 ring-slate-200" />
                ))}
              </div>
            )}
          </article>
